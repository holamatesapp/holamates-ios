...
enum DivisionesLevelEn: String, CaseIterable {
...
struct DivisionesViewEn: View {
    ...
    @State private var level: DivisionesLevelEn = .normal
    ...
    private let LEVEL_SPEEDS: [DivisionesLevelEn: [Double]] = [
                ...
                ScrollView {
                    VStack(spacing: 8) {
                        ...
                        HStack(spacing: 8) {
                            ...
                            Picker("Divisor", selection: $selectedTable) {
                                Text("All").tag("all")

                                ForEach(1...9, id: \.self) { n in
                                    Text("Divisor \(n)").tag("\(n)")   // 👈 mejor sin ":"
                                }
                            }
                            .pickerStyle(.menu)
                            .foregroundColor(.white)
                            .frame(width: 140)   // 🔥 MISMO FIX QUE TABLAS
                            .disabled(running)

                            Picker("Level", selection: $level) {
                                ForEach(DivisionesLevelEn.allCases, id: \.self) { lvl in
                                    Text(lvl.rawValue).tag(lvl)
                                }
                            }
                            .pickerStyle(.menu)
                            .foregroundColor(.white)
                            .frame(width: 140)   // 🔥 MISMO FIX
                            .disabled(running)
                        }
        ...
        ZStack(alignment: .top) {
            ...
            ScrollView {
                VStack(spacing: 24) {
                    ...
                    // TEXTO PRINCIPAL
                    Text("""
    Division operations appear on screen. The first one (blue) is active. Enter the result.
